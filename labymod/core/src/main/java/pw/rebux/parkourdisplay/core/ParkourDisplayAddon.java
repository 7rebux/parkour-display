...
import pw.rebux.parkourdisplay.core.platform.LabyChatOutput;
import pw.rebux.parkourdisplay.core.platform.LabyContext;
import pw.rebux.parkourdisplay.core.platform.LabyInputController;
import pw.rebux.parkourdisplay.core.platform.LabyRunConfig;
...
public final class ParkourDisplayAddon extends LabyAddon<ParkourDisplayConfiguration> {
  ...
  // Wired in enable(), once the LabyMod client APIs the context depends on are available.
  private MinecraftInputUtil minecraftInputUtil;
  private LabyContext context;
  ...
  protected void enable() {
    ...
    this.context = new LabyContext(
        this,
        new LabyInputController(this.minecraftInputUtil),
        new LabyChatOutput(),
        new LabyRunConfig(this)
    );
    this.runState = new RunState(this.context, this.playerState);
    this.macroRunner = new MacroRunner(this.context);
    this.macroRepository = new MacroRepository(DATA_DIR.toPath(), this.gson);
